    ...
    <v-card style="width:450px;" outlined>
        ...
        <v-card-actions>
            ...
            <v-btn
                    ...
                    color="deep-purple lighten-2"
                    text
                    @click="openAccepteOrReject"
            >
                AccepteOrReject
            </v-btn>
            <v-dialog v-model="accepteOrRejectDiagram" width="500">
                <AccepteOrRejectCommand
                        @closeDialog="closeAccepteOrReject"
                        @accepteOrReject="accepteOrReject"
                ></AccepteOrRejectCommand>
            </v-dialog>
            <v-btn
                    v-if="!editMode"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            async accepteOrReject(params) {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links['accepte-or-reject'].href), params)
            ...
            openAccepteOrReject() {
                this.accepteOrRejectDiagram = true;
            },
            closeAccepteOrReject() {
                this.accepteOrRejectDiagram = false;
            },
            async start() {
                try {
                    if(!this.offline) {
